...
# スクレイピング先の情報
shop_name       = "URBAN RESEARCH DOORS"
short_shop_name = "UR DOORS"
target_url      = "http://www.urdoors.com/shoplist/"
...
File.open("seeds.rb", "w") do |file|
  ...
  branch_list_page.css('.ns01c').each do |s|
    ...
    branch_name = short_shop_name + " " +branch_info_page.css('.ttl-shop-name').inner_text.strip
    address = branch_info_page.css('.shop-address').inner_text
