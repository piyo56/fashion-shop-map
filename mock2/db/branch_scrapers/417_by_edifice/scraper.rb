...
# スクレイピング先の情報
shop_name       = "417 EDIFICE"
short_shop_name = "417 EDIFICE"
target_url      = "http://www.baycrews.co.jp/shop_list/417/"
...
File.open("seeds.rb", "w") do |file|
  ...
  branch_list_page.css('.arrow_gray > a').first(10).each do |s|
    branch_name = s.inner_text.strip
    ...
    begin
      ...
      puts "-----> [Error] failed to fetch the page (#{branch_url} | #{branch_name})"
      next
    end

    address = branch_info_page.css('#shopInfo-left > dl > dd').inner_text.strip
