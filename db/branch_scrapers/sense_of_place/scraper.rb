...
# スクレイピング先の情報
shop_name       = "SENSE OF PLACE by URBAN RESEARCH"
short_shop_name = "SENSE OF PLACE"
target_url      = "http://senseofplace.jp/shoplist"
...
File.open("seeds.rb", "w") do |file|
  ...
  branch_list_page.css('.shopdetail').each do |s|
    branch_name = s.css('.name').inner_text.strip
    address     = s.css(".address").inner_text.split(" ")[1]

    begin
      branch_address = /([^a-zA-Z0-9]{1,3}?[都道府県市][^\s]*[0-9]([0-9\-番号丁目])+)/.match(address)[0].strip
