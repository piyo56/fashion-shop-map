...
# スクレイピング先の情報
shop_name       = "FREAK'S STORE"
short_shop_name = "FREAK'S STORE"
target_url      = "https://www.freaksstore.com/area"
...
File.open("seeds.rb", "w") do |file|
  ...
  branch_list_page.css('.shopBox').each do |s|
    branch_url  = target_url + s.attribute("href").value[1..-1]
    branch_name = short_shop_name + " " +s.css('.shopName').inner_text.strip

    address = s.css('.shopInfo').inner_text
